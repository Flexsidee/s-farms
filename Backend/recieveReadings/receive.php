<?php
    ...
    if($api_key == strval($_GET["apikey"])){
        ...
        if($_SERVER["REQUEST_METHOD"] == "GET"){
            ...
            $temperature = sanitizeInput($_GET["temperature"]);
            $humidity = sanitizeInput($_GET["humidity"]);
            $moisture = sanitizeInput($_GET["moisture"]);
    
            //query to insert the data into the database
            $sql = "INSERT INTO data_table (temperature, humidity, moisture) VALUES ('.$temperature.', '.$humidity.', '.$moisture.')";
